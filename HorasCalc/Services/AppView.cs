...
namespace HorasCalc.Services
{
    public static class AppView
    {
        ...
        public static void ShowResults(List<Periodo> periodos)
        {
            ...
            {
                Console.WriteLine($"\t- {periodo.GetPeriodo()} ({periodo.GetDuracao()})\t");
            }
            ...
            if (periodos.Count > 0)
            {
                ...
                foreach (var periodo in periodos)
                {
                    duracaoTotal += periodo.GetDuracaoTimeSpan();
                }

                Console.WriteLine("\n----------------------------------------------------------------------");

                Console.ForegroundColor = ConsoleColor.Blue;
                Console.WriteLine($"Duração total: {duracaoTotal.ToHoraMinuto()} | Azure: {duracaoTotal.ToAzure()}");
            }
            else
            {
